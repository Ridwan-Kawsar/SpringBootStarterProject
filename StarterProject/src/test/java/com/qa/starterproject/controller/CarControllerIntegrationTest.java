...
public class CarControllerIntegrationTest {
	...
	void testCreate() throws Exception {
		Car testCar = new Car("Mclaren", "P4", 2022);
		...
		Car createdCar = new Car(1, "Mclaren", "P4", 2022);
	...
	void testRead() throws Exception {
		List<Car> cars = List.of(new Car(1, "Mclaren", "P4", 2022));
		this.mvc.perform(get("/getCars")).andExpect(status().isOk())
				.andExpect(content().json(this.mapper.writeValueAsString(cars)));
	}

	@Test
	void testUpdate() throws Exception {
		Car updated = new Car(1, "BMW", "I8", 2020);
		this.mvc.perform(patch("/updateCar/1?name=BMW&model=I8&reg=2020")).andExpect(status().isOk())
